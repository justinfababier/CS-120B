 ...
 * 
 * Exercise Description:    The purpose of this exercise is to implement tasks 1 and task 2 of lab 5's syncSM.
 *                          Task 1 implements the ultrasonic sensor.
 *                          Task 2 implements the 4-digit 7-segment display.
 * 
 * I acknowledge all content contained herein, 
 * excluding template or example code, is my own original work.
 * 
 * Demo Link: https://youtu.be/54zHwdZ1zMc
 */
...
const unsigned long SM3_PERIOD = 0;     // Period of state machine 3, zero if not used
const unsigned long SM4_PERIOD = 0;     // Period of state machine 4, zero if not used
...
int main(void) {
    ...
    DDRC = (1 << PC2);    // Set only PC2 as output, others as input
    PORTC = 0x00;         // Initialize PORTC to ensure all outputs are low
